def binary_guess(guesses_allowed, low_num, high_num, correct_num):
    """
    computer makes based on binary search
    ...
    while guesses_allowed != 0:
        ...
        if comp_guess < correct_num:  # TOO LOW
            measure = input(f'Too low, HAL. Guess higher. --------------{num_guesses} guess---{low_num} low---{high_num} high')
            low_num += high_num - comp_guess
            comp_guess = int((high_num - low_num) / 2 + low_num)

        elif comp_guess > correct_num:  # TOO HIGH
            measure = input(f'Too high, HAL. Guess lower. --------------{num_guesses} guess---{low_num} low---{high_num} high')
            high_num -= comp_guess + low_num
            comp_guess = int((high_num - low_num) / 2 + low_num)
